      ...
      <div className="space-y-6 animate-fade-in -mt-8 px-4 md:px-8 pb-8 bg-slate-50/50 min-h-screen">
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-start justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-xl w-full max-w-5xl shadow-2xl flex flex-col h-[650px] max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200 border border-slate-200">
              ...
              <div className="px-8 py-5 border-b border-slate-100 flex justify-between items-center shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-lg font-bold text-slate-800 tracking-tight">{editingProductId ? 'Edit Product Asset' : 'Register New Inventory Asset'}</h2>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{productForm.sku || "Sequential ID Pending"}</p>
                  </div>
                </div>
                <button onClick={() => setIsModalOpen(false)} className="h-8 w-8 flex items-center justify-center rounded-lg hover:bg-slate-50 text-slate-400 transition-all">
                  <X className="h-5 w-5"/>
                </button>
              </div>
              ...
              <div className="flex-1 flex overflow-hidden">
                {/* Minimalist Sidebar */}
                <div className="w-60 border-r border-slate-100 bg-slate-50/30 p-4 space-y-1 overflow-y-auto shrink-0">
                ...
                <div className="flex-1 flex flex-col min-w-0">
                  ...
                  <form onSubmit={handleCreateProduct} className="flex-1 overflow-y-auto px-10 py-10 space-y-10 scrollbar-hide">
                    {activeTab === 'general' && (
                      <div className="space-y-10 animate-in fade-in slide-in-from-right-2 duration-300">
                        ...
                        <div className="grid grid-cols-12 gap-10">
                          ...
                          <div className="col-span-4 space-y-4">
                            ...
                            <div 
                              ...
                              className="aspect-square rounded-xl bg-slate-50 border border-slate-200 flex flex-col items-center justify-center text-slate-400 group hover:border-indigo-400 transition-all cursor-pointer overflow-hidden relative shadow-sm"
                            >
                        ...
                        {!productForm.has_variants && (
                          <div className="pt-8 border-t border-slate-100">
                             <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-500 mb-6">Financial & Stock Inventory</h4>
                             <div className="grid grid-cols-3 gap-8">
                                <Field label="Buying Cost (Price)" type="number" value={productForm.buying_price} onChange={(v: number) => setProductForm({...productForm, buying_price: v})} />
                        ...
                        <div className="pt-4">
                          <Field label="Internal Technical Description" type="textarea" value={productForm.description} onChange={(v: string) => setProductForm({...productForm, description: v})} placeholder="Material specs, weight, or origin details..." />
                        </div>
                      </div>
                    )}
                    ...
                    {activeTab === 'variants' && (
                       <div className="space-y-8 animate-in fade-in slide-in-from-right-2 duration-300">
                          <div className="bg-slate-900 rounded-xl p-8 text-white flex justify-between items-center overflow-hidden relative shadow-lg">
                             <div className="relative z-10">
                                <h3 className="text-xl font-bold tracking-tight">Variation Studio</h3>
                                <p className="text-xs text-slate-400 mt-1">Configure multiple attributes and mass-generate SKUs instantly.</p>
                             </div>
                          ...
                          <div className="grid grid-cols-2 gap-4">
                             {generatorRows.map((row, idx) => (
                                <div key={idx} className="bg-slate-50 p-6 rounded-lg border border-slate-200 space-y-4">
                                   <div className="flex justify-between items-center mb-1">
                                      ...
                                      {generatorRows.length > 1 && (
                                        <button type="button" onClick={() => setGeneratorRows(generatorRows.filter((_, i) => i !== idx))} className="text-slate-400 hover:text-red-500"><X className="h-3 w-3"/></button>
                                      )}
                                   </div>
                                   <div className="flex gap-4">
                                      <div className="flex-1"><Field label="Name (e.g. Color)" value={row.key} onChange={(v: string) => {
                                         const n = [...generatorRows]; n[idx].key = v; setGeneratorRows(n);
                                      }} /></div>
                                      <div className="flex-[2]"><Field label="Values (Comma Sep)" value={row.values} onChange={(v: string) => {
                          ...
                          <div className="flex justify-between items-center pt-2">
                             <div className="flex gap-2">
                                {UNIVERSAL_ATTRIBUTES.map(attr => (
                                   <button 
                                     ...
                                     className="px-3 py-1.5 rounded-md border border-slate-200 text-[10px] font-bold text-slate-500 hover:border-indigo-300 hover:text-indigo-600 transition-all bg-white"
                                   >
                                      + {attr}
                                   </button>
                                ))}
                             ...
                             <div className="flex gap-3">
                                <button type="button" onClick={() => setGeneratorRows([...generatorRows, {key: '', values: ''}])} className="px-5 py-2.5 rounded-lg text-[11px] font-bold text-slate-500 hover:bg-slate-50 transition-all">+ Add Logic Row</button>
                                <button type="button" onClick={generateMatrix} className="px-6 py-2.5 rounded-lg bg-indigo-600 text-[11px] font-bold text-white shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all">Mass Build Matrix</button>
                             </div>
                          </div>
                          ...
                          <div className="pt-10 space-y-4">
                             ...
                             <div className="space-y-3">
                                {variants.map((v, idx) => (
                                   <div key={v.tempId} className="flex gap-6 items-center p-4 rounded-lg border border-slate-200 bg-white group hover:border-indigo-300 transition-all">
                                      <div className="flex-1">
                                         <p className="text-[9px] font-bold uppercase tracking-widest text-slate-400 mb-1">Configuration</p>
                                         <div className="flex flex-wrap gap-1">
                                            {Object.entries(v.attributes || {}).map(([key, val]) => (
                                               <span key={key} className="text-[10px] font-bold px-2 py-0.5 rounded border border-slate-100 bg-slate-50 text-slate-600">
                                                  {key}: {val}
                                               </span>
                                            ))}
                                         </div>
                                      </div>
                                      <div className="w-40"><Field label="Variant SKU" value={v.sku} onChange={(val: string) => {
                                         const n = [...variants]; n[idx].sku = val; setVariants(n);
                                      }} /></div>
                                      <div className="w-28"><Field label="Price" type="number" value={v.selling_price} onChange={(val: number) => {
                                         const n = [...variants]; n[idx].selling_price = val; setVariants(n);
                                      }} /></div>
                                      <div className="w-28"><Field label="Stock" type="number" value={v.stock_quantity} onChange={(val: number) => {
                                         const n = [...variants]; n[idx].stock_quantity = val; setVariants(n);
                                      }} /></div>
                                      <button type="button" onClick={() => setVariants(variants.filter(varnt => varnt.tempId !== v.tempId))} className="text-slate-300 hover:text-red-500 p-2"><Trash2 className="h-4 w-4"/></button>
                                   </div>
                                ))}
                             </div>
                          </div>
                    ...
                    {activeTab === 'units' && (
                       <div className="space-y-8 animate-in fade-in slide-in-from-right-2 duration-300">
                          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-8 flex items-start gap-5">
                          ...
                          <div className="space-y-3">
                             {customUnits.map((unit, idx) => (
                               <div key={idx} className="flex gap-4 items-end p-5 rounded-lg border border-slate-200 bg-slate-50/30">
                                 <div className="flex-1"><Field label="Unit Type" value={unit.unit_name} onChange={(v: string) => {
                                   const next = [...customUnits]; next[idx].unit_name = v; setCustomUnits(next);
                                 }} placeholder="e.g. Packet" /></div>
                                 <div className="w-36"><Field label="Pieces per Unit" type="number" value={unit.conversion_factor} onChange={(v: number) => {
                                   const next = [...customUnits]; next[idx].conversion_factor = v; setCustomUnits(next);
                                 }} /></div>
                                 <div className="w-40"><Field label="Selling Price" type="number" value={unit.selling_price} onChange={(v: number) => {
                                   const next = [...customUnits]; next[idx].selling_price = v; setCustomUnits(next);
                                 }} /></div>
                                 <button type="button" onClick={() => setCustomUnits(customUnits.filter((_, i) => i !== idx))} className="text-slate-400 hover:text-red-500 p-3"><Trash2 className="h-4 w-4"/></button>
                               </div>
                             ))}
                             <button type="button" onClick={() => setCustomUnits([...customUnits, { unit_name: '', conversion_factor: 1, selling_price: 0 }])} className="w-full py-4 rounded-lg border border-dashed border-slate-200 text-xs font-bold text-slate-400 hover:text-indigo-600 hover:border-indigo-300 hover:bg-indigo-50/10 transition-all flex items-center justify-center gap-2">
           ...
           <div className="px-8 py-6 border-b border-slate-100 flex flex-col sm:flex-row gap-6 justify-between items-center bg-white">
              <div className="relative w-full max-w-2xl">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50/50 border border-slate-200 rounded-lg py-3 pl-11 pr-4 text-xs font-semibold placeholder:text-slate-400 focus:bg-white focus:ring-2 focus:ring-indigo-50 focus:border-indigo-500 transition-all outline-none"
                />
              </div>
              <div className="flex items-center gap-3">
                 <button onClick={fetchData} className="px-5 py-2.5 rounded-lg border border-slate-200 bg-white text-[11px] font-bold uppercase tracking-tight text-slate-600 hover:bg-slate-50 transition-all">Sync Registry</button>
                 <button className="h-10 w-10 rounded-lg bg-slate-900 text-white flex items-center justify-center hover:bg-indigo-600 transition-all shadow-md"><Filter className="h-4 w-4"/></button>
              </div>
           </div>
                      ...
                      <tr key={p.id} className="group hover:bg-slate-50/30 transition-all">
                        <td className="px-8 py-6">
                           <div className="flex items-center gap-6">
                              <div className="h-12 w-12 rounded-lg bg-slate-50 border border-slate-200 flex items-center justify-center shadow-sm overflow-hidden shrink-0">
                                ...
                                {p.variants && p.variants.length > 0 && (
                                   <div className="flex flex-wrap gap-1 mt-1.5">
                                      {p.variants?.slice(0, 3).map((v: any, i) => (
                                         <span key={i} className="text-[8px] font-bold px-1.5 py-0.5 rounded border border-slate-100 bg-white text-slate-500 uppercase">
                              ...
                              <button 
                                onClick={() => {
                                  if (confirm('Permanently remove this asset from the vault?')) {
                                    productService.delete(p.id).then(() => fetchData());
                                  }
                                }}
